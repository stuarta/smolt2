class Processor::DeviceProcessor
  def find_or_create_device!(device)
    ...
    begin
      device_bus = Core::DeviceBus.create_or_find_by!(name: device["bus"])
    rescue ActiveRecord::RecordNotUnique
      device_bus = Core::DeviceBus.find_by(name: device["bus"])
    end
    if device["type"].nil?
      begin
        device_class = Core::DeviceClass.create_or_find_by!(name: "NONE")
      rescue ActiveRecord::RecordNotUnique
        device_class = Core::DeviceClass.find_by(name: "NONE")
      end
    else
      begin
        device_class = Core::DeviceClass.create_or_find_by!(name: device["type"])
      rescue ActiveRecord::RecordNotUnique
        device_class = Core::DeviceClass.find_by(name: device["type"])
      end
    end
    begin
      d = Core::Device.create_or_find_by!(vendor_id: device["vendor_id"],
